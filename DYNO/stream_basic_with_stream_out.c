...
int main()
{
	...
	// Channels/Addresses to stream in. NUM_IN_CHANNELS can be less than or
	// equal to the size of CHANNEL_IN_NAMES
...
void HardcodedConfigureStream(int handle)
{
	...
	printf("    Setting AIN_ALL_RANGE to %d\n", AIN_ALL_RANGE);
...
void StreamOutAndIn(int handle, const int numInChannels,
	...
	StreamOutChannel soChannel, double scalar)
{
	...
	char streamOutBufferF32[50];
	char streamOutSetLoop[50];
	sprintf(streamOutLoopSize, "STREAM_OUT%d_LOOP_SIZE", soOffset);
	sprintf(streamOutBufferF32, "STREAM_OUT%d_BUFFER_F32", soOffset);
	...
	WriteNameArrayOrDie(handle, streamOutBufferF32, soChannel.targetLoopSize,
	    aValues);
